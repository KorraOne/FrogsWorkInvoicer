...
def _sync_block_message():
    ...
    return subscription_messages.SYNC_REQUIRED.format(days=days)
...
def check_generate_access():
    ...
    if not billing_auth_store.is_authenticated():
        return False, "account_required", subscription_messages.TRIAL_GATE
    ...
    if cache.get("active") is False and cache.get("last_verified_at"):
        return False, "subscription_inactive", subscription_messages.SUBSCRIPTION_INACTIVE

    if entitlement_cache.sync_status() in ("grace_expired", "never_verified"):
        return False, "sync_required", _sync_block_message()

    return False, "subscribe_required", subscription_messages.SUBSCRIBE_REQUIRED
...
def preview_context(format_money):
    ...
    sync_state = entitlement_cache.sync_status()
    reminder = None
    if billing_auth_store.is_authenticated() and sync_state == "reminder":
        reminder = subscription_messages.SYNC_REMINDER
