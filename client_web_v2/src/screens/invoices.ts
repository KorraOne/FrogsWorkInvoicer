...
export async function renderInvoices(panel: HTMLElement, ctx: AppContext): Promise<void> {
  ...
  panel.innerHTML = `
    ...
    ${
      totalCount
        ? `${renderGroup("Not sent yet", "not_sent", groups.not_sent, settings, multiBiz)}
           ${renderGroup(`Sent, awaiting payment (${formatMoney(sentTotal)} owing)`, "sent", groups.sent, settings, multiBiz)}
           ${renderGroup("Paid", "paid", groups.paid, settings, multiBiz)}`
        : emptyStateHtml(
            "No invoices yet",
