...
Open http://127.0.0.1:8080/admin and log in with `ADMIN_PASSWORD` from `/etc/frogswork/billing.env`.

Sessions expire after `ADMIN_SESSION_HOURS` (default 8). Use **Log out** on shared machines.
...
| `FLASK_SECRET_KEY` | Yes | Random 32+ character secret |
